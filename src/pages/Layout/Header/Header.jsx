...
const Header = () => {
    ...
    return (
        <>
                ...
                <div className="container">
                    <div className="header__nav">
                        ...
                        <ul className="header__list">
                            {
                                icons.map(item => (
                                    <li key={item.id} onClick={() => nav(`${item.reference}`)}
                                        className={`header__cube ${location.pathname === `/${item.pageName}` ? 'active' : ''}`}>
                                        {
                                            item.pageName === 'favorites' ? <span
                                                className="header__circle">{data.filter(item => item.favorite).length}</span> : item.pageName === 'cart' ?
                                                <span
                                                    className="header__circle">{data.filter(item => item.cart).length}</span> : item.pageName === 'comparison' ?
                                                    <span
                                                        className="header__circle">{data.filter(item => item.comparison).length}</span> : ''
                                        }
                                        {
                                            item.icon
                                        }
                            ...
                            {
                                data.length && <li onClick={() => nav(`/admin`)}
                                                   className={`header__cube ${location.pathname === `/admin` ? 'active' : ''}`}>
                ...
                <div className="header__pages">
                    <div className="container">
                        <div className="header__under">
                            <div style={{background: catalog ? '#F05A00' : ''}} onClick={() => catalogFunc()}
                           ...
                           <span className="header__icon">
                            ...
                            <label style={{width: search ? '542px' : '310px'}} className="header__label">
                                ...
                                <input style={{borderRadius: search && '5px 5px 0 0'}} value={search}
                                       onChange={(e) => setSearch(e.target.value)} placeholder={t('header.search')}
                                       type="text" className="header__field"/>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <div style={{
                display: catalog ? 'flex' : 'none',
                position: 'absolute',
                rowGap: 'none',
                zIndex: '30',
                left: '110px'
            }} className="category__navigation">
                <div className="category__navigation__top">
                    {
                        categoryForMain.map(item => (
                            <li style={{
                                background: turnOn && text === item.textEN ? '#212526' : 'white',
                                color: turnOn && text === item.textEN ? 'white' : 'black'
                            }} key={item.id} onClick={() => board(item.textEN)} className="category__navigation__line">
                                {
            ...
            <div style={{display: turnOn ? 'flex' : "none", top: '185px', left: '420px'}} className="category__board">
                {
                    categoryForMain.filter(item => (
                        ...
                        filteredItem.subdivisions.map(subdivision => (
                            <li onClick={() => nav(`/catalog`)} key={subdivision.id} className="category__board__line"
